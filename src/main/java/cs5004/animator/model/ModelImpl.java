...
/**
 * This class implements class of the model interface, getting shape and its animation
 * of the graphics.
 */
public class ModelImpl implements Model {
  ...
  /**
   * Add an animation of appearing to the List of Animations. Animation must have a name, type,
   * and start/end time.
   */
  @Override
  public void action(String name, ActionType actionType, int startTime, int endTime) {
    ...
    for (Animation a : animations) {
      if (a.getShapeAndName().getName().equals(name) && a.getActionType() == actionType) {
    ...
    for (ShapeSet s : CurrentState) {
      if (s.getName().equals(name) && actionType == ActionType.AppearDisappear) {
  ...
  /**
   * Add an animation of moving to the List of Animations. Animation must have a name, type,
   * move to point, and start/end time.
   */
  public void action(String name, ActionType actionType,
    ...
    for (ShapeSet s : CurrentState) {
      if (s.getName().equals(name) && actionType == ActionType.Move) {
  ...
  /**
   * Add an animation of color change to the List of Animations. Animation must have a name, type,
   * color to change to, and start/end time.
   */
  public void action(String name,ActionType actionType, Color colorTo, int startTime, int endTime) {
    ...
    for (ShapeSet s : CurrentState) {
      if (s.getName().equals(name) && actionType == ActionType.Color) {
  ...
  /**
   * Add an animation of scale change to the List of Animations. Animation must have a name, type,
   * scaled factor, and start/end time.
   */
  public void action(String name, ActionType actionType,
    ...
    for (ShapeSet s : CurrentState) {
      if (s.getName().equals(name) && actionType == ActionType.Scale) {
        ShapeSet copied = s.copy();
        animations.add(new Animation(copied, actionType, new ShapeSet(s.getName(),
      ...
      switch (shape.getShape().getType()) {
        case rectangle:
          s += "Create " + shape.getShape().getColorToString() + " rectangle "
              + shape.getName() + " with corner at " + shape.getShape().getReference()
        ...
        case oval:
          s +=  "Create " + shape.getShape().getColorToString() + " oval "
              + shape.getName() + " with center at " + shape.getShape().getReference()
      ...
      switch (a.getActionType()) {
        ...
        case Scale:
          switch (a.getShapeAndName().getShape().getType()) {
            case rectangle:
              s +=  "Shape " + a.getShapeAndName().getName() + " changes from Width: "
              ...
              break;
            case oval:
              s += "Shape " + a.getShapeAndName().getName() + " changes from X Radius: "
                  + a.getShapeAndName().getShape().getUniqueIdentifierOne()
  ...
  /**
   * get the current state of the Shape given the name.
  ...
  public List<ShapeSet> getShapesAtTick(int time) {
    ...
    for (ShapeSet shape : shapes) {
      ...
      int blue;
      for (Animation a : animations) {
        if (a.getShapeAndName().getName().equals(shape.getName())
      ...
      if (!hasAnimation) {
        ...
        if (shape.getShape().getType() == ShapeType.oval) {
          s.add(new ShapeSet(shape.getName(),
