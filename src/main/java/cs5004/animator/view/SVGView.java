...
public class SVGView implements IView {
   ...
   * @param s toString read.
   * @return svg string output.
   */
  @Override
  public String showSVGString(String s) {
    StringBuilder output = new StringBuilder();
    Map<String, StringBuilder> dict = new LinkedHashMap<>();
    String[] lns = s.split("\n");
    int i;
    String[] background = lns[0].split(" ");
    output.append("<svg viewBox=\"").append(background[4]).append(" ")
        .append(background[6]).append(" ").append(background[8]).append(" ")
        .append(background[10])
    ...
    for (i = 1; i < lns.length; i++) {
      ...
      if (line.length > 0) {
        switch (line[0]) {
          case "Create":
            String[] p = line[7].substring(1, line[7].length() - 1).split(",");
            if (line[2].equals("oval")) {
              dict.put(line[3], new StringBuilder());
              dict.get(line[3]).append("<ellipse id=\"").append(line[3]).append("\" cx=\"")
                  .append(p[0]).append("\" cy=\"").append(p[1]).append("\" rx=\"")
                  .append(Integer.parseInt(line[10]) / 2).append("\" ry=\"")
                  .append(Integer.parseInt(line[12]) / 2).append("\" fill=\"rgb").append(line[1])
              ...
              dict.get(line[3]).append("<rect id=\"").append(line[3]).append("\" x=\"")
                  ...
                  .append(line[10]).append("\" height=\"").append(line[13]).append("\" fill=\"rgb")
              ...
              case "moves":
                dict.get(line[1]).append("    <set attributeName=\"visibility\" attributeTyp"
                    ...
                    .append((Integer.parseInt(line[12].substring(2))
                        ...
                        * speed).append("ms\" fill=\"freeze\" />\n");
                String[] f = line[4].substring(1, line[4].length() - 1).split(",");
                String[] t = line[6].substring(1, line[6].length() - 1).split(",");
                if (dict.get(line[1]).substring(1,2).equals("r")) {
                  dict.get(line[1]).append("    <animate attributeType=\"xml\" begin=\"")
                          .append(Integer.parseInt(line[9].substring(2)) * speed)
                          .append("ms\" dur=\"").append((Integer.parseInt(line[12]
                          .substring(2)) - Integer
                          .parseInt(line[9].substring(2))) * speed)
                      .append("ms\" attributeName=\"x\" from=\"").append(f[0]).append("\" to=\"")
                          .append(t[0]).append("\" fill=\"freeze\" />\n");
                  dict.get(line[1]).append("    <animate attributeType=\"xml\" begin=\"")
                          .append(Integer.parseInt(line[9].substring(2)) * speed)
                          .append("ms\" dur=\"").append((Integer.parseInt(line[12]
                          .substring(2)) - Integer.parseInt(line[9]
                          .substring(2))) * speed)
                          .append("ms\" attributeName=\"y\" from=\"").append(f[1])
                      .append("\" to=\"").append(t[1]).append("\" fill=\"freeze\" />\n");
                }
                else {
                  dict.get(line[1]).append("    <animate attributeType=\"xml\" begin=\"")
                          .append(Integer.parseInt(line[9].substring(2)) * speed)
                          ...
                          .substring(2)) - Integer.parseInt(line[9]
                          .substring(2))) * speed)
                      .append("ms\" attributeName=\"cx\" from=\"").append(f[0]).append("\" to=\"")
                          .append(t[0]).append("\" fill=\"freeze\" />\n");
                  dict.get(line[1]).append("    <animate attributeType=\"xml\" begin=\"")
                          .append(Integer.parseInt(line[9].substring(2)) * speed)
                          .append("ms\" dur=\"").append((Integer.parseInt(line[12]
                          .substring(2)) - Integer.parseInt(line[9]
                          .substring(2))) * speed)
                          .append("ms\" attributeName=\"cy\" from=\"").append(f[1])
                      .append("\" to=\"").append(t[1]).append("\" fill=\"freeze\" />\n");
                }
                break;
              case "changes":
                if (line[3].equals("color")) {
                  dict.get(line[1]).append("    <animate attributeName=\"fill\" "
                      ...
                      .append("\" to=\"rgb").append(line[7])
                          .append("\" begin=\"").append(Integer.parseInt(line[10]
                          .substring(2)) * speed).append("ms\" dur=\"")
                      .append((Integer.parseInt(line[13].substring(2))
                          - Integer.parseInt(line[10].substring(2))) * speed)
                  ...
                  dict.get(line[1]).append("    <set attributeName=\"visibility\" "
                      ...
                      .append("ms\" dur=\"").append((Integer.parseInt(line[18]
                          .substring(2)) - Integer.parseInt(line[15]
                          .substring(2))) * speed).append("ms\" fill=\"freeze\" />\n");
                  if (dict.get(line[1]).toString().charAt(1) == 'r') {
                    dict.get(line[1]).append("    <animate attributeName=\"width\" "
                        ...
                        .append("ms\" fill=\"freeze\" from=\"")
                        .append(line[5].substring(0, line[5].length() - 1))
                        .append("\" to=\"").append(line[10].substring(0, line[10].length() - 1))
                        .append("\" />\n");
                    dict.get(line[1]).append("    <animate attributeName=\"height\" "
                            + "attributeType=\"XML\" begin=\"").append(Integer
                            .parseInt(line[15].substring(2)) * speed).append("ms\" dur=\"")
                        .append((Integer.parseInt(line[18].substring(2))
                            - Integer.parseInt(line[15].substring(2))) * speed)
                        .append("ms\" fill=\"freeze\" from=\"").append(line[7])
                    ...
                    dict.get(line[1]).append("    <animate attributeName=\"rx\" "
                        ...
                        .append("ms\" dur=\"").append((Integer.parseInt(line[18]
                            .substring(2)) - Integer.parseInt(line[15]
                            .substring(2))) * speed).append("ms\" fill=\"freeze\" from=\"")
